# Agent Directives for Design OS (Hermes Agent Edition)
...
Design OS follows a structured planning sequence. Each step is a Hermes skill:

### 1. Product Vision (`/skill design-os-product-vision`)
...
Use `/skill design-os-product-roadmap`, `/skill design-os-data-shape` individually to update those files after initial creation.

### 2. Design System (`/skill design-os-design-tokens`)
...
### 3. Application Shell (`/skill design-os-design-shell`)
Design the persistent navigation and layout that wraps all sections.
**Output:** `product/shell/spec.md`, `src/shell/components/`

### 4. For Each Section:
- `/skill design-os-shape-section` — Define the specification and generate sample data + types
- `/skill design-os-sample-data` — Update sample data and types (if already created)
- `/skill design-os-design-screen` — Create screen designs
- `/skill design-os-screenshot-design` — Capture screenshots

### 5. Export (`/skill design-os-export-product`)
...
The `/skill design-os-export-product` skill generates a UI design handoff package:
